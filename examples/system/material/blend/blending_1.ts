...
let dstMaterial = new TextureMaterial({
  ...
  textures: {
    color: dstTexturePremultiplied  //Texture 类
  }
});
...
function update() {
  ...
  if (settings.alphaMode == "premultiplied") {
    ...
    //   alpha: clear.alpha
    // });
    scene.backgroudColor = [...clear.color, clear.alpha];
    //alpha预乘
    if (clear.premultiply) {
      scene.premultipliedAlpha = true;
    } else {
      scene.premultipliedAlpha = false;
    }
  }
  else {
    scene.backgroudColor = [...clear.color, 1];
    // scene.setBackgroudColor({
    //   red: clear.color[0],
    //   green: clear.color[1],
    //   blue: clear.color[2],
